...
➕ Crear (INSERT)
docker exec -it america1 cockroach sql --insecure -e "INSERT INTO megamarket.usuario (email,password_hash,nombre,region) VALUES ('cliente_usa@test.com','hash','Juan USA','AMERICA');"

📖 Leer (SELECT)

Ver todos los usuarios:

docker exec -it america1 cockroach sql --insecure -e "SELECT \* FROM megamarket.usuario;"

Obtener IDs para luego modificar:

docker exec -it america1 cockroach sql --insecure -e "SELECT id_usuario, email, nombre FROM megamarket.usuario;"

✏️ Modificar (UPDATE)

Reemplazar AQUI_EL_UUID por un identificador real:

docker exec -it america1 cockroach sql --insecure -e "UPDATE megamarket.usuario SET nombre='USUARIO EDITADO', password_hash='updated' WHERE id_usuario='AQUI_EL_UUID';"

🗑️ Eliminar (DELETE)
docker exec -it europa1 cockroach sql --insecure -e "DELETE FROM megamarket.usuario WHERE nombre='USUARIO EDITADO';"

🛠️ Validación de replicación

Después de insertar, actualizar o eliminar datos, ejecutar:

docker exec -it europa1 cockroach sql --insecure -e "SELECT \* FROM megamarket.usuario;"
...
Validar que los datos sigan sincronizados tras el retorno del nodo.
